 ...
 * Exported for reuse in aggressive cleanup.
 */
export function convertKnownHtmlTags(text: string): { content: string; changesCount: number } {
	let changesCount = 0;
	let processed = text;
	...
	// Strip known formatting-only tags with no markdown equivalent
...
/**
 * Strip all remaining HTML tags (orphan open/close tags).
 * Exported for reuse in aggressive cleanup.
 */
export function stripRemainingHtmlTags(text: string): { content: string; changesCount: number } {
	let changesCount = 0;
...
function convertHtmlToMarkdown(content: string): RuleResult {
	...
	const result = processOutsideCode(content, (text) => {
		...
		// Note: Standard tier does NOT strip unknown tags - preserves custom elements
	});
